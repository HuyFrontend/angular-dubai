...
import { InfoComponentProfileData, ProfileLinkedData, Page, LinkedItem } from './info-component-profile.model';
import { MBCConfirmationComponent } from "components/mbcConfirmation";
import { INFO_COMPONENT_FIELD, PAGE_STATUS } from 'constant';


@Component({
  selector: "info-component-profile",
  styleUrls: ["info-component-profile.scss"],
  templateUrl: "info-component-profile.html",
  providers: [PageSubTypeService]
})
export class InfoComponentProfileComponent implements OnInit, OnChanges, AfterViewInit {

  @Input() mainPageEntityId: string;
  @Input() pageInfo: PageInfo;
  @Input() dataInput: InfoComponentProfileData;
  @Input() isFormSubmitted: boolean;
  @Output() infoComponentChange = new EventEmitter<InfoComponentProfileData>();
  ...
  public icData: InfoComponentProfileData;
  ...
  constructor(
    ...
    this.icData = new InfoComponentProfileData();
  ...
  instantiateProfileLinkedData() : ProfileLinkedData {
    ...
    const linkedItem2 = this.instantiateProfileLinkedItem2();
    const linkedData =  new ProfileLinkedData(undefined, linkedItem1, linkedItem2);
    linkedData._props = {};
    return linkedData;
  }

  instantiateProfileLinkedItem2(){
    return  new LinkedItem(undefined, null, null, undefined);
  }

  ngOnInit() {
    if(this.icData.linkDataBOList.length == 0){
      this.icData.linkDataBOList.push(this.instantiateProfileLinkedData());
    }
    const cachePageConfigs = this.localStorageService.retrieve(storageConfigs.page);
    this.pageSubTypeService.getDataFromCache('profileSubTypes', cachePageConfigs)
      .subscribe(data => {
  ...
  fixICData(data: InfoComponentProfileData){
    data.linkDataBOList = data.linkDataBOList.map( linkedData => {
      ...
      const linkedItem2 = linkedData.linkedItem2 || this.instantiateProfileLinkedItem2();
      const linkedDataInstance = new ProfileLinkedData(linkedData.entityId, linkedData.linkedItem1, linkedItem2);
  ...
  onQueryItem1Page2({val, updateEvent}, subType) {
    ...
    this.pageService
        .suggest(new PageSuggestionRequest("linkedPageForIcProfile", val, 'profile', subType, excludeIds, excludedStatus))
        .subscribe(pages => {
  ...
  onItem1Page2Change(entry, rowData: ProfileLinkedData){
    if(entry){
  ...
  onItem2Page2PChange(entry, rowData: ProfileLinkedData){
   ...
   */
  onAddItem2Page2(entry, rowData: ProfileLinkedData) {
   ...
   */
  onRemoveItem2Page2(entry, rowData: ProfileLinkedData) {
   ...
   */
  onQueryItem2({val, updateEvent}, rowData: ProfileLinkedData){
    const _updateEvent: BehaviorSubject<any> = updateEvent;
    ...
    if(rowData.linkedItem2.linkType !== 'METADATA'){
      ...
      this.pageService
          .suggestPageMetadata(val, 'profile', rowData.linkedItem1.linkType)
   ...
   * add item to metadata list
   */
  onAddItem2Metadata(entry, rowData: ProfileLinkedData) {
   ...
   * remove an item on metadata list
   */
  onRemoveItem2MetaData(entry, rowData: ProfileLinkedData) {
    rowData.linkedItem2.linkProperties.metadata = rowData.linkedItem2.linkProperties.metadata.filter(val => val != entry);
    this.onInfoControlChanged();
  }

  onRelationSelectboxChange( value, rowData: ProfileLinkedData){
  ...
  onPageSubTypeChanged(linkedData: ProfileLinkedData){
  ...
  revertChangingPageSubType(linkedData: ProfileLinkedData){
